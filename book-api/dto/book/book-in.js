...
const BookPagination = (validators, className) => {
  return classCreator(
    class extends Validator {
      ...
      @validators(IsOptional(), IsObject('by must be object'))
      by;
    },
    className
  );
...
const BookCreate = (validators, className) => {
  return classCreator(
    class extends Validator {
      ...
      @validators(IsString('name must be string'))
      name;

      @validators(IsPositive('publishedTime must be a number'))
      publishedTime;
      ...
      @validators(IsId('categoryId must be numeric string and contains 13 character'))
      categoryId;

      @validators(IsBase64Image('avatar must be files!'))
      avatar;

      @validators(IsArray('string', 'images must be an array!'), IsBase64Image('images must be files!'))
      images;
...
const PdfFileSaved = (validators, className) => {
  return classCreator(
    class extends Validator {
      ...
      @validators(IsString('pdf must be string!'))
      pdf;
    },
    className
  );
...
const BookSave = (validators, className) => {
  return classCreator(
    class extends Validator {
      @validators(IsId('bookId must be numeric string and contain 13 character', { groups: 'update' }))
      bookId;
      ...
      @validators(IsIds('authors must be an id or an id array!'))
      authors;
    },
    className
  );
...
const BookFileCreated = (validators, className) => {
  return classCreator(
    class extends Validator {
      @validators(IsMulterFile('pdf must be files!'))
      pdf;

      @validators(IsMulterFile('avatar must be files!'))
      avatar;

      @validators(IsArray('object', 'images must be an array!'), IsMulterFile('images must be files!'))
      images;
    },
    className
  );
...
const IntroduceHTMLFileSave = (validators, className) => {
  return classCreator(
    class extends Validator {
      ...
      @validators(IsString('html must be a string!'))
      html;

      @validators(IsString('json must be a string!'))
      json;

      @validators(IsString('fileName must be a string!'))
      fileName;
    },
    className
  );
...
const BookAuthors = (validators, className) => {
  return classCreator(
    class extends Validator {
      @validators(IsArray('object', 'authors must be an array!'))
      authors;
    },
    className
  );
