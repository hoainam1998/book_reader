...
const { IsString, IsBase64Image, IsObject, IsGraphqlSelect, IsId } = require('#decorators/validators');
...
const CategoryValidator = (validators, className) => {
  return classCreator(
    class extends Validator {
      @validators(IsId('categoryId must be numeric string and contain 13 character', { groups: ['update'] }))
      categoryId;
...
const CategoryDetailValidator = (validators, className) => {
  return classCreator(
    class extends Validator {
      @validators(IsId('categoryId must be numeric string and contain 13 character'))
      categoryId;

      @validators(IsObject('Query must be object!'), IsGraphqlSelect('Value of field must be boolean!'))
      query;
    },
    className
  );
